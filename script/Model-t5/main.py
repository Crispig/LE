...
def main():
    ...
    parser.add_argument('--continue_training',
                        default=False,
                        action='store_true',
                        help='continue_training or not.')
    parser.add_argument('--divide_data',
    ...
    data_file = {'train':'train',  'valid':'dev', 'test':'test', 'train1':'train1',  'train2':'train2',
                 'train3':'train3', 'train4':'train4'}
    ...
    train_data = None
    if args.do_train:
        train_data = data_utils.Loader(args.data_dir, data_file['train1'], args.cache_size, args.train_batch_size, device)
        num_train_steps = int(
    ...
    if not args.continue_training:
        model = chose_model_model(args.bert_model,args)
        # print(model)
    else:
        # model = torch.load(args.load_path + '/' + args.bert_model + '_acc8697.bin')
        model = torch.load('/home/inspur/asc20/LE/fpz/cloze_model/output/bert-large-uncased_acc8071.bin')
        #tokenizer = chose_model_token(args.bert_model,args)
    ...
    if args.load_model:
        if args.ngpu>1:
            print('        model is loading......   PATH:'+args.load_path+'/'+args.bert_model+'_'+str(args.ngpu)+'.bin')
            model = torch.load(args.load_path+'/'+args.bert_model+'_'+str(args.ngpu)+'.bin')
        else:
            print('        model is loading......   PATH:'+args.load_path+'/'+args.bert_model+'.bin')
            model = torch.load(args.load_path+'/'+args.bert_model+'-1'+'.bin')
    ...
    if args.do_train:
        ...
        for _ in range(int(args.num_train_epochs)):
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                valid_data = data_utils.Loader(args.data_dir, data_file['valid'], args.cache_size, args.eval_batch_size,
                                               device)
                ...
                for inp, tgt in valid_data.data_iter(shuffle=False):
                    ...
                    with torch.no_grad():
                        tmp_eval_loss, tmp_eval_accuracy, _ = model(inp, tgt)
                    if n_gpu > 1:
                ...
                if args.save_model:
                    if eval_accuracy > max_eval_accuracy:
                        ...
                        if args.ngpu > 1:
                            print('        model is saving......   PATH:' + args.load_path + '/' + args.bert_model + '_' + str(
                                args.ngpu) + '.bin')
                            torch.save(model, args.load_path + '/' + args.bert_model + '_' + str(args.ngpu) + '.bin')
                        else:
                            print('        model is saving......   PATH:' + args.load_path + '/' + args.bert_model + '.bin')
                            torch.save(model, args.load_path + '/' + args.bert_model +'-1'+ '.bin')
        ...
        np.save(save_history_path + '/'+args.bert_model + '.loss_history.npy', loss_history)  # 保存为.npy格式
        np.save(save_history_path + '/' + args.bert_model + '.acc_history.npy', acc_history)  # 保存为.npy格式

        # 读取
        # a = np.load('a.npy')
        # a = a.tolist()

    if args.do_eval and not args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        logging("***** Running evaluation *****")
        logging("  Batch size = {}".format(args.eval_batch_size))
        valid_data = data_utils.Loader(args.data_dir, data_file['valid'], args.cache_size, args.eval_batch_size,
                                       device)
