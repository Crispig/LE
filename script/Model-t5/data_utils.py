...
class Preprocessor(object):
    def __init__(self, args, device='cpu'):
        # self.tokenizer = t5Tokenizer.from_pretrained(args.bert_model)
        self.tokenizer = T5Tokenizer.from_pretrained("./pre_training/" + args.bert_model + "/" +
        ...
        for i in range(len(self.data_objs)):
            ...
            # for j in self.data_objs[i].article:
            #     print(self.tokenizer._convert_id_to_token(int(j.item()))[1:], end=' ')
            # break
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='t5 cloth')
    args = parser.parse_args()
    data_collections = ['train', 'dev', 'train1', 'train2', 'train3', 'train4']
    # data_collections = ['train1', 'train2', 'train3', 'train4']
    for item in data_collections:
        args.data_dir = './ELE/{}'.format(item)
        args.pre = args.post = 0
        args.bert_model = 't5-large'
        args.save_name = './data/{}-{}.pt'.format(item, args.bert_model)
        # args.save_name = './data/{}-{}.pt'.format(item, "funnel-xlarge")
