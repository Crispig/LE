...
if __name__ == "__main__":
    ...
    model5 = torch.load('../../model/'+model_name+'-all.bin', map_location=torch.device('cpu'))
    ...
    if isinstance(model5, torch.nn.DataParallel):
    ...
    model4.eval()
    model5.eval()
    f1 = open('../result/t5-large-1.txt','w')
    f2 = open('../result/t5-large-2.txt','w')
    f3 = open('../result/t5-large-3.txt','w')
    f4 = open('../result/t5-large-4.txt','w')
    f5 = open('../result/t5-large-all.txt','w')
    ft = open('../result/t5-large-tgt.txt','w')

    tokenizer = T5Tokenizer.from_pretrained('./pre_training/' + model_name + '.model')
    def eval(file='./ELE/dev/dev0056.json'):
        ...
        for k in range(len(sample_list)):
            ...
            for inp, tgt in load.data_iter():
                ...
                loss5,acc5,out5 = model5(inp,tgt)
                ...
                print(out5.detach().numpy().tolist(),file=f5)
