...
class T5PredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super(T5PredictionHeadTransform, self).__init__()
...
class T5ForCloze(T5PreTrainedModel):
    def __init__(self, config):
        super(T5ForCloze, self).__init__(config)
        self.t5 = T5ForConditionalGeneration(config)
        # self.cls = T5OnlyMLMHead(config)
        self.init_weights()
        self.loss = nn.CrossEntropyLoss(reduction='none')
        self.vocab_size = self.t5.shared.weight.size(0)
    ...
    def forward(self, inp, tgt):
        ...
        bsz = ops.size(0)
        opnum = ops.size(1)
        out = self.t5(input_ids=articles, attention_mask=articles_mask, decoder_input_ids=articles,
                      decoder_attention_mask=articles_mask)  # ,output_all_encoded_layers=False)
        out = out[0]
