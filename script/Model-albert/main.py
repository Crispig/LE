...
#-------------正文--------------
    ...
    data_file = {'train':'train',  'valid':'dev', 'test':'test'}
    ...
    train_data = None
    if args.do_train:
        train_data = data_utils.Loader(args.data_dir, data_file['train'], args.cache_size, args.train_batch_size, device)
        num_train_steps = int(
...
#-----------------load_model---------------------
    if args.load_model:
        if args.ngpu>1:
            ...
            model = torch.load(args.load_path+'/'+args.bert_model+'.bin')
...
# ---------------save_mode------------------
    if args.save_model:
        if args.ngpu>1:
            print('        model is saving......   PATH:'+args.load_path+'/'+args.bert_model+'_'+str(args.ngpu)+'.bin')
            ...
            torch.save(model,args.load_path+'/'+args.bert_model+'.bin')
...
### -------------------------------do_eval ------------------------------------
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        valid_data = data_utils.Loader(args.data_dir, data_file['valid'], args.cache_size, args.eval_batch_size, device)
