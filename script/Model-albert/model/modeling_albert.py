...
from model_config.configuration_albert import AlbertConfig
...
# AlbertPreTrainedModel 加载预训练模型及参数
...
'''
MLMHead 也就是某个词用 [mask] 替换掉，而后用模型预测这句话中的每一个词，最后只计算 [mask] 词的损失

'''
class AlbertMLMHead(nn.Module):
    def __init__(self, config):
        super(AlbertMLMHead, self).__init__()
        ## 这里是MLM任务的  现将drop从0.3->0
        ...
        self.activation = ACT2FN[config.hidden_act]
        self.decoder.bias = self.bias   # 编码的bias
        # print("this is AlbertMLMHead parameters")   
        # print("self.bias {}".format(self.bias.size()))
        # print("self.dense {}".format(self.dense.size()))
        # print("self.decoder {}".format(self.decoder.size()))
...
class AlbertForMaskedLM(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # print("this is AlbertMLMHead model")   
...
class ALbertForCloze(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        print("build for ALbertForCloze")
    ...
    '''
    accuracy计算准确度
    out求出输出的最后一维的最大值与标准答案
    '''
    def accuracy(self, out, tgt):  #target
        # print("tgt.shape {} ".format(tgt.size()))
        # print("out.shape {} ".format(out.size()))
        # 取最后一维的max值
        out = torch.argmax(out, -1)
        # print("最后计算准确度时的out: out {} ".format(out))
        # exit()
...
# 前向传播
    def forward(self, inp, tgt):
        ...
        # print("*****ALbertForCloze parameters.shape*******")
        # print("articles.shape:{}".format(articles.size()))
        # print("articles_mask.shape:{}".format(articles_mask.size()))
        # print("ops.shape:{}".format(ops.size()))
        # print("ops_mask.shape:{}".format(ops_mask.size()))
        # print("ops_mask.shape内容：{}".format(ops_mask))
        # print("question_pos.shape:{}".format(question_pos.size()))
        # print("question_pos问题空的具体位置:{}".format(question_pos))
        # print("mask.shape:{}".format(mask.size()))
        # print("mask的具体位置{}".format(mask))
        ...
        bsz = ops.size(0)
        opnum = ops.size(1)
        out, _ = self.albert(articles, attention_mask=articles_mask)
        # print("\n从albert模型出来的out的形状{}\n".format(out.size()))
        ...
        # print("拓展维数后 question_pos.shape:{}".format(question_pos.size()))

        out = torch.gather(out, 1, question_pos)
        # print("经过gather后的out.shape:{}".format(out.size()))
        # print("经过gather后的out {}".format(out))

        # 这里的prediction就是从主模型到30000词的映射
        out = self.predictions(out)
        # print("经过predictions后 的out out.shape:{}".format(out.size()))
        # print("经过prediction(MLM)后的out{}".format(out))

        # convert ops to one hot 将选项转换成onehot编码
        # torch.view就是reshape()   .view
        out = out.view(bsz, opnum, 1, -1)   
        out = out.expand(bsz, opnum, 4, -1)
